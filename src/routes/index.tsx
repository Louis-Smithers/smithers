...
const TITLE = "Smithers — Operations & Automation Consulting for Canadian Small Businesses";
const DESCRIPTION =
...
export const Route = createFileRoute("/")({
  ...
  component: SmithersLanding,
});

function SmithersLanding() {
  return (
...
/** Section heading with an inline mono counter prefix.
 *  Not an "eyebrow" stack — number + title sit on the same baseline. */
...
// Color is the brand's signature hex (no `#`). Used both for the Simple Icons
// CDN fill and for the wordmark fallback if the logo doesn't load. Notion and
// Retool are naturally black/white — we render them in bone so they stay
...
function ToolMark({ name, slug, color }: Tool) {
  ...
  // Try Simple Icons first, then a JSDelivr mirror. If both fail (a few brands
  // have been pulled from Simple Icons for trademark reasons), render a colored
  // brand-letter badge so every cell stays the same shape.
  const tryNextOrFail = () => {
